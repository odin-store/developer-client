...
const MagicCard: React.FC<MagicCardProps> = ({
  ...
  size = 600,
  spotlight = true,
  borderColor = "hsla(160, 16%, 100%, 0.79)",
  isolated = true,
  highlighted,
  ...props
}) => {
  return (
    <div
      ...
      className={cn(
        ...
        `bg-[radial-gradient(var(--mask-size)_circle_at_var(--mouse-x)_var(--mouse-y),var(--border-color),transparent_80%)]`,
        className
      )}
      {...props}
    >
      ...
      <div
        className={
          "absolute inset-[1.5px] -z-20 rounded-2xl bg-white dark:bg-black/95"
        }
      />
    </div>
  );
